...
def class_prediction(image_path, model, topk):
    ...
    1. Predicting classes - the script reads in an image and a checkpoint then
    prints the most likely image class and its associated probability
    2. Top K classes - the script allows users to print out the top K classes
    along with associated probabilities

    Args:
    image_path: str. Directory of image to predict.
    model: The trained model.
    topk: int. The top k probability classes.
    
    Returns:
    probabilities: float.
    classes: str. The class label of the image.
    '''
